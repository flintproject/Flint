...
class ReplacingPrinter : public boost::static_visitor<> {
public:
	void operator()(const Compound &c) const {
		printf("(%s", c.keyword.c_str());
		std::vector<Expr>::const_iterator bit = c.children.begin();
					  ...
					  const Expr &rhs)
{
	if (lhs.which() == kExprIsString) {
		...
		printf("%s %s#0 ", uuid.c_str(), id.c_str());
		boost::apply_visitor(ReplacingPrinter(), rhs);
		putchar('\n');
	} else {
		...
		printf("%s %s#0 (plus %s (times @dt ",
			   uuid.c_str(), id.c_str(), id.c_str());
