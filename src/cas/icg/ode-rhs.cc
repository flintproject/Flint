...
OdeRhs::OdeRhs(Printer *printer, sqlite3 *db)
	: printer_(printer)
...
OdeRhs::~OdeRhs() = default;

bool OdeRhs::Insert(const boost::uuids::uuid &uuid,
					const Def &def)
{
					...
					const Ode &ode)
{
	...
	std::string name = oss.str();
	printer_->set_uuid(uuid);
	boost::apply_visitor(*printer_, ode.rhs());
	std::string math = printer_->GetAndClearString();
	if (!inserter_->Insert(uuid, name.c_str(), math.c_str()))
		return false;
