...
#define TESTCASE1(f, a, expected) do {									\
		...
		BOOST_CHECK(runtime::Init(driver_.db(), 0, "layout", bytecode.get(), nullptr, nullptr, &data)); \
...
BOOST_AUTO_TEST_CASE(same_seed)
{
	...
	BOOST_CHECK(runtime::Init(driver_.db(), 42, "layout", bytecode.get(), nullptr, nullptr, &data0));
	std::vector<double> data1;
	BOOST_CHECK(runtime::Init(driver_.db(), 42, "layout", bytecode.get(), nullptr, nullptr, &data1));
...
BOOST_AUTO_TEST_CASE(different_seed)
{
	...
	BOOST_REQUIRE(bytecode);
	std::vector<double> data0;
	BOOST_CHECK(runtime::Init(driver_.db(), 0, "layout", bytecode.get(), nullptr, nullptr, &data0));
	std::vector<double> data1;
	BOOST_CHECK(runtime::Init(driver_.db(), 1, "layout", bytecode.get(), nullptr, nullptr, &data1));
